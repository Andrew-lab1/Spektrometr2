...
PxLSetFrameBufferPolicy(
   IN HANDLE hCamera,
   IN U32    uNonTriggeredFrames,
   IN U32    uTriggeredFrames,
   IN U32    uTotalFrameBufferSizeInMs);
#endif /* PIXELINK_LINUX */
...
PxLGetStreamState(
   IN HANDLE  hCamera,
   OUT PULONG pStreamState,
   OUT PULONG pNumberFrameBuffers);
#endif
...
PXL_RETURN_CODE
PXL_API
PxLGetNextCompressedFrame(
   IN HANDLE       hCamera,
   IN U32          bufferSize,
   OUT LPVOID      pFrame,
   OUT FRAME_DESC* pFrameDesc,
   OUT COMPRESSION_DESC*  pCompressionDesc,
   IN OUT U32*     compressionDescSize);

PXL_RETURN_CODE
PXL_API
PxLDecompressFrame(
   IN void const * pSrcFrame,
   IN FRAME_DESC * pSrcFrameDesc,
   IN COMPRESSION_DESC const *  pCompressionDesc,
...
PXL_API
PxLSetEventCallback ( 
    IN HANDLE hCamera,  /* Camera identifier */
    IN U32    eventId,  /* Event of interest */
    IN LPVOID pContext, /* Context pointer to be supplied to the callback */
    IN U32 (PXL_APICALL* EventProcessFunction)( /* Callback function */
            IN HANDLE             hCamera,         /* Camera identifier */
            IN U32                eventId,         /* The event that 'occurred' */
            IN double             eventTimestamp,  /* Time when the event occured */
            IN U32                numDataBytes,    /* Number of data bytes associated with the event (if any) */
            IN OUT LPVOID         pData,           /* Data associated with the event */
            IN LPVOID             pContext));      /* Context pointer supplied when the callback was set */
...
/*
 * Functions used with Action commands
 */
PXL_RETURN_CODE 
PXL_API
PxLSetActions(
    IN U32     actionType,
    IN double  scheduledTimestamp);

PXL_RETURN_CODE 
PXL_API
PxLGetActions(
    IN HANDLE      hCamera,
    OUT double*    pScheduledTimestamps,
    IN OUT U32*    pNumberOfTimestamps);
