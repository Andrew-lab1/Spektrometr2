...
#define ApiSuccessSuspectedFirewallBlockWarning ((PXL_RETURN_CODE)0x0000000B)  /* Difficuties receiving data from the camera -- suspected firewall issue */
#define ApiSuccessApiLiteWarning          ((PXL_RETURN_CODE)0x0000000C) /* Only used with PxLInitialize(Ex) -- Camera sucessfully initialized with Lite variant of the API */
#define ApiSuccessSerialPortScanSuppressedWarning ((PXL_RETURN_CODE)0x0000000D) /* Could not enumerate all controllers, as com port scans are suppressed via the registry SuppressSerialPortScan */
#define ApiSuccessSensorsCannotSyncWhileStreaming ((PXL_RETURN_CODE)0x0000000E) /* Sensors can only be synchronized while the stream is stopped/paused */
#define ApiFeatureDeprecatedWarning       ((PXL_RETURN_CODE)0x0000000F) /* Maintained for backwards compatibility, but functionality has been replaced */
#define ApiCompressionNotPossibleWarning  ((PXL_RETURN_CODE)0x00000010) /* Compression is enabled, but current configuration does not support it.*/
...
#define ApiGetNextFrameBusy          ((PXL_RETURN_CODE)0x80000019)  /* PxLGetNextFrame() cannot be called at this time, there is a */
                                                                    /* camera / API conflict as to how frames are delivered -- by calling */
                                                                    /* PxLGetNextFrame or via a FRAME_OVERLAY callback function */
...
#define ApiUnsupportedClipEncoding   ((PXL_RETURN_CODE)0x90000021)  /* Attempt to capture a video clip using an unsupported video encoding scheme */
#define ApiVideoEncodingError        ((PXL_RETURN_CODE)0x90000022)  /* The compression libraries reported an error while captureing video */
#define ApiVideoFrameTooLargeError   ((PXL_RETURN_CODE)0x90000023)  /* The video frame is too large for the copression library (h264 is limited to 9 MegaPixels) */
#define ApiVideoInsufficientDataError ((PXL_RETURN_CODE)0x90000024) /* There was not enough data to encode */
...
#define ApiThisEventNotSupported     ((PXL_RETURN_CODE)0x9000002B)   /* Attempted to enable a specific event on a camera that does not support it */
#define ApiFeatureConflictError      ((PXL_RETURN_CODE)0x9000002C)   /* Attempt to enable a feature that uses a resource currently in use by another feature */
#define ApiGpiOnlyError              ((PXL_RETURN_CODE)0x9000002D)   /* Attempt to treat a dedicated GPI as a GPO */
#define ApiGpoOnlyError              ((PXL_RETURN_CODE)0x9000002E)   /* Attempt to treat a dedicated GPO as a GPI */
#define ApiInvokedFromIncorrectThreadError   ((PXL_RETURN_CODE)0x9000002F)  /* This operation cannot be done from this thread */
#define ApiNotSupportedOnLiteVersion ((PXL_RETURN_CODE)0x90000030)   /* 'Lite' version only -- Attempt to use functionlaity only supported with full API */
#define ApiControllerDuplicateType   ((PXL_RETURN_CODE)0x90000031)   /* Attempt to assign a cotnroller to a camera that already has a controller assigned of the same TypeMask */
#define ApiControllerDuplicateFeature ((PXL_RETURN_CODE)0x90000032)  /* Attempt to assign a cotnroller to a camera that already has native support of the cotrollers TypeMask */
#define ApiCompressionNotPossibleError ((PXL_RETURN_CODE)0x90000033) /* Attempt to stream compressed images, but the current configuration is not compatible with compression */
#define ApiDecompressionNotPossibleError ((PXL_RETURN_CODE)0x90000034) /* Attempt to stream compressed images, but the host system is not compatible with compression */
#define ApiNotSupportedIn32BitError  ((PXL_RETURN_CODE)0x90000035)   /* Attempt to perfrom an operation from a 32-bit appliction, that is only supported in 64-bit */
#define ApiRequiresUncompressedFrameError ((PXL_RETURN_CODE)0x90000036)   /* Attempt to perform an operation with a compressed frame; the frame should be decopressed first */
#define ApiMemoryBufferAlignmentError ((PXL_RETURN_CODE)0x90000037)  /* The requested operation has memory buffer alignment restrictions that are not being met  */

// Aliases for backwards compatibility
#define ApiOhciDriverError                 ApiLinkDriverError
#define ApiNotPermittedWhileStreamingError ApiNotPermittedWhileStreaming
#define ApiH264EncodingError               ApiVideoEncodingError    
#define ApiH264FrameTooLargeError          ApiVideoFrameTooLargeError   
#define ApiH264InsufficientDataError       ApiVideoInsufficientDataError  
