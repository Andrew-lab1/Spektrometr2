...
void PxLSlider::activate(bool writeable)
{
    gtk_widget_set_sensitive (m_scale, writeable);
    gtk_widget_set_sensitive (m_value, writeable);
}
...
void PxLSlider::setValue(double value)
{
    ...
    // TEMP PEC +++
    //     the below 'fix' would not allow us to update the edit box to a camera rounded
    //    value.  Remove for now, and check to see if C-OEM needs an alternative fix for this bug.
    //if (! (gtk_widget_get_state_flags(m_value) & GTK_STATE_FLAG_FOCUSED))
    {
       if (value < 10)
       {
           // dealing with small values, so show 4 decimal places
           sprintf (cValue, "%5.4f",value);
